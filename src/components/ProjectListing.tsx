  ...
  return (
    <div className="flex flex-col gap-8">
      {projects.map(
        (project) =>
          project && (
            <Fragment key={project.githubUrl}>
              <ProjectEntry
                title={project.title}
                description={project.description}
                slug={project.githubUrl}
                created={
                  project.createdAt ? project.createdAt.toDateString() : ""
                }
...
function ProjectEntry({
  ...
  return (
    <Link
      target="_blank"
      href={slug}
      className="hover:cursor-pointer hover:underline flex justify-between"
    >
      <div className="flex gap-2 px-8 flex-col flex-nowrap text-justify justify-center">
        ...
        <div className="flex flex-wrap gap-4">
          {languages.map((l, i) => (
            <div key={l.name + i} className="bg-pink-800 rounded-md py-1 px-2">
              {l.name}
            </div>
          ))}
        </div>
      </div>
      <SlArrowRight className="self-center text-3xl" />
    </Link>
  );
}
